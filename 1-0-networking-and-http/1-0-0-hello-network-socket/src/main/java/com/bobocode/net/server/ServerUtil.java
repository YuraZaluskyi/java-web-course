...
/**
 * Util class for a {@link MessageBoardServer}. It provides all necessary method for network
 * communication. Using these utils you can create server socket, accept connection from client,
 * read String message from client socket and print it using special provided format.
 */
...
//        throw new ExerciseNotCompletedException(); // todo: implement according to javadoc and verify by ServerUtilTest
  ...
  /**
   * This is the most important method of this class. It allows reading all messages sent by client
   * form the sockets. In order to read from a socket, it uses its {@link InputStream}. But since we
   * want to read {@link String} messages instead of bytes, it creates a {@link BufferedReader}
   * based on the socket {@link InputStream}. Using the reader, it reads a line and returns a {@link
   * String} message.
   *
...
//    throw new ExerciseNotCompletedException(); // todo: implement according to javadoc and verify by ServerUtilTest
  ...
  /**
   * Simple message that allows to print message using a nice and informative format. Apart from the
   * text message itself, it also prints the time and client address.
   *
